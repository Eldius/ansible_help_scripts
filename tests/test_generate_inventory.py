...
from generate_inventory import get_hostname, generate_inventory
...
async def test_generate_inventory_full_flow(tmp_path):
    ...
    with patch("generate_inventory.get_hosts", AsyncMock(return_value=mock_discovered)), \
         patch("generate_inventory.get_hostname", side_effect=mock_get_hostname):
